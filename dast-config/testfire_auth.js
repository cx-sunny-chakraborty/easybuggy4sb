...
function authenticate(helper, paramsValues, credentials) {
    print("Authenticating TestFire via script...");

    var loginPageUrl = paramsValues.get(getLogin); // login.jsp
    var loginPostUrl = "http://testfire.net/doLogin";

    var username = credentials.getParam(usernameParam);
    var password = credentials.getParam(passwordParam);

    // STEP 1: GET login page (init session)
    var initRequest = {
        method: HttpRequestHeader.GET,
        uri: new URI(loginPageUrl, false),
        body: null,
        contentType: null,
        cookies: null
    };

    var initResponse = authenticatePhase(helper, initRequest, false);

    // Extract session cookie
    var cookies = getCookiesFromResponse(initResponse);

    // STEP 2: POST login
    var requestBody = paramsValues.get(userNameKey) + "=" + encodeURIComponent(username) +
                      "&" + paramsValues.get(passwordKey) + "=" + encodeURIComponent(password);

    var loginRequest = {
        method: HttpRequestHeader.POST,
        uri: new URI(loginPostUrl, false),
        body: requestBody,
    ...
    var loginResponse = authenticatePhase(helper, loginRequest, false);
